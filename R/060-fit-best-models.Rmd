...
```{r}
### Get the R2
rsq_results <- map(.x = results_list, ~rsq(.x, vdgcp_slope48m, .pred)) %>%
  enframe() %>%
...
### Get the RMSE
rmse_results <- map(.x = results_list, ~rmse(.x, vdgcp_slope48m, .pred)) %>%
  enframe() %>%
  bind_cols(model = model_df) %>%
  unnest(cols = c(value)) %>%
  select(model, model_fct, .estimate) %>%
...
predictions %>%
  filter(model %in% models_to_present) %>%
  ggplot(aes(x = vdgcp_slope48m, y = .pred, color = model_fct)) +
    geom_point() +
